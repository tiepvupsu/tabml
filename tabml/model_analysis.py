...
class ModelAnalysis:
    ...
    Attributes:
        ...
        mapping_dict: Dict[str, Dict[str, str]]
            A dictionary of {feature_name: {encoding_feature_name: raw_feature_name}}
            to store label encoded feature mappings.
    """
    ...
    def __init__(
        ...
        self.mapping_dict = self._get_mapping_dict()
    ...
    # TODO: Add analyze for Keras model and tensorflow dataset
    def analyze(self):
    ...
    def _get_dataset(self, dataset_name: str):
        if dataset_name == "train":
            return self.data_loader.get_train_ds()
        elif dataset_name == "val":
            return self.data_loader.get_val_ds()
        else:
    ...
    def _get_predictions(self, dataset) -> Dict[str, Iterable]:
        ...
        df_input.pop(self.label_col)
        res = {}
        if self.need_predict:
            res[self.pred_col] = self.model_wrapper.predict(df_input)
        if self.need_predict_proba:
            res[self.pred_proba_col] = self.model_wrapper.predict_proba(df_input)
        return res
    ...
    def _get_metric_dict(
        ...
        # For overall or non-encoding features
        raw_feature_value = self.mapping_dict[feature_name].get(
            feature_value, feature_value
        )

        res = {
            f"encoded_{feature_name}": feature_value,
            f"raw_{feature_name}": raw_feature_value,
    ...
    def _get_feature_mapping(self, feature_name):
        mapping_file_name = os.path.join(
            self.data_loader.mapping_dir, f"{feature_name}.csv"
        )
        if not os.path.isfile(mapping_file_name):
            # If the is no mapping_file_name, then return and empty dict. This dict
            # will be later used as an identity dictionary, i.e. no encoding.
            return {}
        df = pd.read_csv(mapping_file_name)
        mapping = {key: val for key, val in zip(df.encoding_value, df.raw_value)}
        return mapping

    def _get_mapping_dict(self):
        return {
            feature: self._get_feature_mapping(feature)
